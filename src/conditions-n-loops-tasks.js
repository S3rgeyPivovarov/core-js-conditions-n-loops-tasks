...
function convertNumberToString(numberStr) {
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    ...
    switch (sym) {
      ...
      default:
        if (sym >= '0' && sym <= '9') {
          const digit = parseInt(sym, 10);
          wordStr += `${digitWords[digit]} `;
        }
        break;
    }
  }
  ...
  let wordsStrTrim = '';
  for (let i = 0; i < wordStr.length; i += 1) {
    if (i !== wordStr.length - 1) {
      wordsStrTrim += wordStr[i];
    }
  }
...
function rotateMatrix(/* matrix */) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    let right = n - 1;
    while (left < right) {
      const temp = matrixT[i][left];
      matrixT[i][left] = matrixT[i][right];
      matrixT[i][right] = temp;
      left += 1;
      right -= 1;
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    let evenChars = '';
    let oddChars = '';
    for (let j = 0; j < n; j += 2) evenChars += newString[j];
    for (let j = 1; j < n; j += 2) oddChars += newString[j];
